...
const GamePage: React.FC<GamePageProps> = ({ stops }) => {
  ...
  return (
    <div className="w-screen h-screen flex flex-col bg-slate-950 text-white overflow-hidden">
      {/* Top Bar */}
      <nav className="h-16 flex items-center justify-between px-6 glass z-[1000] border-b border-white/10 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-black tracking-tighter text-lg leading-none">ESHOT PUZZLE</h1>
      ...
      <div className="flex-1 flex relative overflow-hidden">
        {/* Sidebar */}
        <aside 
          className={`absolute left-0 top-0 bottom-0 z-[999] w-80 glass border-r border-white/10 transition-transform duration-500 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        >
          <div className="p-6 h-full flex flex-col">
            <header className="mb-6">
              <h2 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-1">Şu Anki Durak</h2>
              <p className="text-xl font-bold leading-tight line-clamp-2">{gameState.currentStop.durak_adi}</p>
            </header>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {availableLines.length > 0 ? (
                      ...
                      <p className="col-span-2 text-slate-500 text-sm italic py-4">Bu durakta hat bulunamadı.</p>
                    )}
                  </div>
                </section>
              ) : (
            ...
            <footer className="mt-6 pt-6 border-t border-white/10">
              <div className="bg-orange-500/10 border border-orange-500/20 rounded-2xl p-4">
                <p className="text-[10px] font-black text-orange-400 uppercase mb-1">Hedef</p>
                <p className="text-sm font-bold truncate">{stops[1].durak_adi}</p>
              </div>
            </footer>
          </div>
